...
for sequence in ['lhcb1', 'lhcb2']:
    ...
    if sequence == 'lhcb1':
        ...
        lref = xt.Line.from_json('/home/sterbini/2023_03_06_xsuite_test/lhcmask/python_examples/'
                                'hl_lhc_collisions_python_offbb_onerrors_b2/xsuite_lines/line_bb_for_tracking.json')
    ...
    for nn in ltest.element_names:
        if nn.startswith('mss.'):
            assert np.isclose(ltest[nn].ksl[2], lref[nn].ksl[2], atol=ks2_max*0.11, rtol=0)
